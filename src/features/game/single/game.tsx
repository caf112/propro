...
export const CodeGame = () => {
  ...
  const getUserCode = () => {
    const processCode = (codeLines: string[]) => {
      return codeLines
        .map((line) => {
          return line.replace(/\[\[blank_(\d+)\]\]/g, (_, p1) => {
            // 'match' を削除
            const blankId = `blank_${p1}`
            const blank = problemData.blanks.find(
              (b: Blank) => String(b.id) === blankId || b.id === String(p1), // 型の不一致を解消
            )
      ...
      if (showSolution) {
        ...
        return (
          <div key={index}>
            ...
            <select value={userAnswer} onChange={(e) => handleChange(blank.id, e.target.value)}>
