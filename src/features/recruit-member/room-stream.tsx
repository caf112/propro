...
export const RecruitMember = ({ roomId }: { roomId?: string }) => { 
  ...
  return (
    <div style={{ maxWidth: "600px", margin: "0 auto", textAlign: "center", fontFamily: "Arial, sans-serif" }}>
      {!isLoading ? (
        <>
          <button onClick={() => refetch} style={buttonStyle("#6c757d", "#5a6268")}>Refresh</button>
          <p><strong>あいことば:</strong></p>
          ...
          {currentRoom?.members?.length ? (
            <ul style={{ listStyle: "none", padding: 0, marginTop: "10px" }}>
              {currentRoom.members.map((member, index) => (
                <li key={member?.username} style={{ display: "flex", alignItems: "center", gap: "10px", marginBottom: "10px" }}>
                  <img
